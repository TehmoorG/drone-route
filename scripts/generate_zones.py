...
import osmnx as ox
from src.zone_utils import (
    create_zones_gdf,
    save_zones_to_geojson,
    plot_zones,
    clear_existing_maps,
)
...
def get_user_input():
    ...
    visualize = (
        input("Would you like to visualize the zones? (yes/no): ").strip().lower()
    )

    if visualize == "yes":
        # If visualization is chosen, ask for the place name to download the map
        place_name = input(
            "Enter the name of the place (e.g., 'Accra, Ghana'): "
        ).strip()
    ...
    while True:
        ...
        no_fly_zones.append(
            {"latitude": float(lat), "longitude": float(lon), "radius": float(radius)}
        )
...
def main():
    ...
    save_zones_to_geojson(
        no_fly_zones_gdf, os.path.join(map_directory, "no_fly_zones.geojson")
    )

    # Save the avoidance zones GeoDataFrame to GeoJSON
    save_zones_to_geojson(
        avoidance_zones_gdf, os.path.join(map_directory, "avoidance_zones.geojson")
    )
